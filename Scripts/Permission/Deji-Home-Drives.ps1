...
foreach ($Folder in $GCIHomeDriveAdminFolder)
{
	...
	foreach ($PrhUser in $FolderPrh)
	{
		...
		if ($FolderName -eq $PrhUser)
		{
			...
			$Value = """$FolderPath""" + "," + """$FolderName""" + "," + """$FolderACLTopPermissionsUsername"""
			...
			{
				Add-Content -Path "C:\Deji - PUKHomeDriveDenyAccess.csv" -Value $Value
			}
			else
			{
				
				Add-Content -Path "C:\Deji - Not PUKHomeDriveDenyAccess.csv" -Value $Value
			}
		}
	}
}
